...
| **Night lights** (DMSP-OLS + VIIRS) | ~~Needs an EOG account; the two instruments are not comparable without a harmonised product~~ — **superseded 2026-08-19**: the harmonised splice is published under CC BY 4.0 over 1992-2024 and needs no account (Figshare 9828827 v10, verified in `methods/covariate-survey-2026-08-addendum.md` §4) | Explanatory | The factor with the most specific mechanism for *nocturnal* migration, which is this project's own response variable. The work that remains is not an account and a splice but a *break test*: the DMSP-to-VIIRS join is an instrument change inside the record, and this project has been wrong about two of those. |
