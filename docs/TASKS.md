...
| 27 | The globe drawn in the same hand | The basemap is the last part of the page that is not drawn. Land as a rough.js pencil hatch through `fill-pattern` and `map.addImage`, a wobbled graticule, and the Natural Earth coastlines pre-jittered into two overlapping passes at build time — capped by zoom, because the jitter is fixed in degrees and above about zoom 4 it would put a shoreline in the wrong place. Not the data layers: a circle is a measurement, and wobbling it draws a shape the data does not have. |
